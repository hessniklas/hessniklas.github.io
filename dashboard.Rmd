...
max_temp = clean_noaa_df %>%
  plot_ly(y = ~tmax, color = ~factor(year, ordered = TRUE), type = "box", colors = "viridis")
...
month_temp = clean_noaa_df %>%
  ...
  plot_ly(
    ...
    color = ~factor(year, order = TRUE), text = ~text_label, alpha = 0.5, colors = "viridis")

month_temp
```

### Chart C - GGPLOT Object showing Scatterplot with Min_Temperatures and Max_Temperatures by Year

```{r}
maxmin_plot = clean_noaa_df %>%
  ggplot(aes(x = tmax, y = tmin, color = year)) +
  geom_point() +
  labs(title = "Max vs Min by year",
       x = "Max Temp (C)",
       y = "Min Temp (C)",
       caption = "Data from NY NOAA.") +
  theme(legend.position = "left")

ggplotly(maxmin_plot)
```
